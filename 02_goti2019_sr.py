...
Goti et al. (2019) — Symbolic Regression Training and Evaluation
"""
...
VP_GOTI  = 38.4   # peripheral volume [L] — fixed
Vp_fn    = lambda row: VP_GOTI

CSV_PATH = "ML_Dataset_GotiFullFinal.csv"

# Features selected for SR (from prior feature importance analysis)
CL_FEATURES = ["Age", "Scr.Baseline", "BUN", "Weight",
                "Albumin", "Gender", "Mean.SBP", "RDW"]
...
# Output filenames
CL_OUTPUT = "goti_SR_CL_final.xlsx"
VC_OUTPUT = "goti_SR_Vc_final.xlsx"
...
def sr_CL(row) -> float:
    return 4.971738 / (
        (row["Scr.Baseline"] / (
            row["Weight"] / (
                (row["Gender"] * 19.06421) + (row["BUN"] + row["Age"])
            )
        )) + 0.4163539
    )

def sr_Vc(row) -> float:
    return row["Weight"] * (0.99782836 ** row["Weight"])
...
c_trough_ref = compute_ref_troughs(
    filtered_data, cov_CL, cov_Vc, Vp_fn=Vp_fn, Q=Q_GOTI
)
